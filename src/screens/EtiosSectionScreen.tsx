...
import type { EtiosRow } from "@/types/production";
import { Save, ArrowRight } from "lucide-react";

const MODELS = ["hbk", "sdn"] as const;
const MODEL_LABELS = { hbk: "HBK", sdn: "SDN" };

export default function EtiosSectionScreen() {
  const { currentEntry, updateEtios } = useProduction();
  const navigate = useNavigate();
  const [rows, setRows] = useState<EtiosRow[]>(currentEntry?.etios ?? []);

  useEffect(() => {
    if (currentEntry) setRows(currentEntry.etios);
  }, [currentEntry]);
  ...
  const handleSave = async () => {
    try {
      await updateEtios(rows);
    } catch (err) {
      console.error("Failed to save etios:", err);
    }
  };
  ...
  return (
    <ScreenLayout title="Etios Section" showBack actions={
      ...
      <div className="overflow-x-auto rounded-xl border border-border shadow-card bg-card">
        <table className="w-full min-w-[500px] text-xs lg:text-sm border-collapse">
          <thead>
            <tr className="bg-primary text-primary-foreground">
              <th className="py-2.5 px-3 text-left font-semibold min-w-[120px]">Part</th>
              {MODELS.map(m => (
          ...
          <tbody>
            {rows.map((row, idx) => (
              <tr key={row.part} className={`${idx % 2 === 0 ? "bg-card" : "bg-muted/50"} hover:bg-muted/70 transition-colors`}>
                <td className={`py-2.5 px-3 font-medium text-foreground ${idx % 2 === 0 ? "bg-card" : "bg-muted/50"}`}>{row.part}</td>
                {MODELS.map(m => (
                  <>
